...
describe 'Puppet::Util::PuppetdbValidator' do
  ...
  before do
    ...
    conn_ok = stub()
    conn_ok.stubs(:get).with('/v1/metrics/mbean/java.lang:type=Memory', {"Accept" => "application/json"}).returns(response_ok)
    conn_ok.stubs(:read_timeout=).with(2)
    conn_ok.stubs(:open_timeout=).with(2)

    conn_not_found = stub()
    conn_not_found.stubs(:get).with('/v1/metrics/mbean/java.lang:type=Memory', {"Accept" => "application/json"}).returns(response_not_found)
